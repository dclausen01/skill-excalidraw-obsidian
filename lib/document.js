 ...
 * an der jeweils nächsten "##"-Überschrift oder an der "%%"-Zeile — je
 * nachdem, was zuerst kommt. Fehlt die Überschrift, ist die Sektion leer.
 *
 * Ohne diese Eingrenzung griffen die Sektions-Regexes auf das gesamte
 * Dokument zu und lasen dabei auch Fließtext innerhalb von Textelementen
 * (z. B. Obsidian-Blockreferenzen wie "^abc12345" im Elementinhalt selbst)
 * fälschlich als Indexeinträge.
 */
function abschnitt(markdown, ueberschrift) {
  ...
  const ende = rest.match(/^(?:## .*|%%)$/m);
  return ende ? rest.slice(0, ende.index) : rest;
}

/**
 * sceneToMarkdown trennt die Blöcke der Text Elements-Sektion durch je eine
 * Leerzeile (teile.push(`${el.rawText} ^${el.id}`, "")). Innerhalb eines
 * Blocks kann rawText selbst mehrzeilig sein — nur die jeweils LETZTE Zeile
 * trägt die Blockreferenz "^id", alle vorherigen Zeilen sind reiner
 * Elementinhalt. Bloßes Scoping auf die Sektion reicht deshalb nicht: Ein
 * Textelement wie "See issue ^abc12345\nSecond line here" landet vollständig
 * innerhalb der Sektion, und seine erste Zeile sieht für sich genommen wie
 * ein Indexeintrag aus. Diese Funktion zerlegt die Sektion in ihre Blöcke und
 * liefert je Block nur die letzte, nicht-leere Zeile — genau die Zeile, die
 * laut Serialisierungsformat den Marker tragen kann.
 */
...
export function markdownToScene(markdown) {
  ...
  const textElementeAbschnitt = abschnitt(markdown, "## Text Elements");
