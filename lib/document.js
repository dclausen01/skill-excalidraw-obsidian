 ...
 *
 * `files` bleibt standardmäßig leer — die geschriebene .excalidraw.md löst
 * Bilder über die "## Embedded Files"-Sektion auf (Spec 2.5.1). Nur der
 * Render-Pfad braucht die tatsächlichen Bilddaten inline; dafür `mitBilddaten:
 * true` setzen, was `files` aus `szene.embeddedFiles()` befüllt. Geparste
 * Szenen-Objekte (z. B. aus markdownToScene) haben kein `embeddedFiles` —
 * daher der Guard.
 */
export function sceneToObject(szene, { pluginVersion = readPluginVersion(), mitBilddaten = false } = {}) {
  const files = {};
  if (mitBilddaten && typeof szene.embeddedFiles === "function") {
    for (const [fileId, embed] of szene.embeddedFiles()) {
      files[fileId] = {
...
export function sceneToMarkdown(szene, { pluginVersion = readPluginVersion() } = {}) {
  ...
  // Element Links-Sektion für verlinkte Elemente
  const mitLink = elements.filter((el) => el.link);
  if (mitLink.length > 0) {
    teile.push("## Element Links", "");
    for (const el of mitLink) teile.push(`${el.id}: ${el.link}`, "");
  }

  // Embedded Files-Sektion: löst Bilder für die geschriebene Datei über den
  // Dateinamen auf (files bleibt in der Datei selbst leer, siehe sceneToObject).
  // Guard wie in sceneToObject: manche Aufrufer (z. B. Test-Stubs mit nur
  // `elements()`) haben kein embeddedFiles.
  const embeds = typeof szene.embeddedFiles === "function" ? szene.embeddedFiles() : new Map();
  if (embeds.size > 0) {
    teile.push("## Embedded Files", "");
    for (const [fileId, embed] of embeds) teile.push(`${fileId}: [[${embed.dateiname}]]`, "");
  }
