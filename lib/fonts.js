...
export function loadFontRegistry(dir = FONT_DIR) {
  ...
  return {
    ...
    /**
     * Prüfungsfreie Variante von fontFor: ob ein Zeichen in fontFamily abgedeckt ist,
     * ohne bei Nichtabdeckung zu werfen. lib/text.js steuert damit die Ersatzbreiten-
     * Logik, ohne über try/catch auf fontFor angewiesen zu sein — fontFor selbst bleibt
     * unverändert und wirft weiterhin, denn der Fallback gehört nicht in die
     * Schriftauflösung.
     */
    covers(codepoint, fontFamily) {
      const eintrag = proFamilie.get(fontFamily);
      if (!eintrag) throw new Error(`Unbekannte fontFamily ${fontFamily} — erzeugt werden nur 5 und 6`);
      return eintrag.zeichenKarte.has(codepoint);
    },
