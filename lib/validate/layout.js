...
export function checkGeometry(elemente, befunde) {
  ...
  // Überlappungen zwischen eigenständigen Nicht-Frame-Elementen. Pfeile werden
  // hier zusätzlich ausgeschlossen: arrowElement() (connect.js) speichert x/y als
  // Startpunkt und width/height als absolute Ausdehnung, nicht als normale
  // Top-Left-AABB — bei einem nach links/oben laufenden Pfeil zeigt diese Box
  // in die falsche Richtung und überlappt scheinbar die Form, mit der der Pfeil
  // verbunden ist. Ein Pfeil, der eine Form kreuzt, mit der er verbunden ist,
  // ist keine Kompositions-Auffälligkeit, sondern by design (Schlussprüfung,
  // Finding 2).
  const koerper = eigenstaendig.filter((e) => e.type !== "frame" && e.type !== "arrow");
