...
import { checkSchema, checkReferences, checkTextIndex, detectOutOfScope } from "./structure.js";
 ...
 * Elementtypen/Schriften bestehen, die dieser Skill selbst erzeugt; sonst
 * `{ fremdeTypen, fremdeSchriften }` (siehe detectOutOfScope in structure.js).
 * Für solche Szenen werden Geometrie-, Textpassungs- und Lesbarkeitsprüfung
...
export function validateScene(elemente, { markdown = null, registry = loadFontRegistry(), zoomL0 = 1 } = {}) {
  ...
  const istFremd = fremdesVorkommen.fremdeTypen.length > 0 || fremdesVorkommen.fremdeSchriften.length > 0;
