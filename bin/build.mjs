...
// Ein Renderfehler (z. B. Chromium-Absturz, kaputtes Bündel) darf den Vault nicht
// erreichen — daher fängt der äußere try/catch ihn ab und beendet mit einer
// verständlichen Zeile statt eines rohen Stack-Trace. Das innere try/finally
// schließt den Renderer in jedem Fall, bevor der Fehler weitergereicht wird.
if (!kennzeichen.has("--skip-render")) {
  ...
  try {
    ...
    console.error(`\nAbbruch: Rendering fehlgeschlagen — ${fehler.message}. Es wurde nichts geschrieben.`);
    process.exit(1);
  }
}
...
try {
  ...
  console.error(`\nAbbruch: Schreiben nach ${zielPfad} fehlgeschlagen — ${fehler.message}.`);
  process.exit(1);
}
